...
namespace lab5
{
    class Program
    {
        ...
        static void My_Binary()
        {
            ...
            //and divide variable by 2 in the loop WHATTTTTTT ?????????????????????????????????????????????????? 
            ...
            if (int.TryParse(Console.ReadLine(), out num))
            {
                ...
                Console.WriteLine($"binary string is: {Convert.ToString(num, 2)}");

                Console.WriteLine($"Use modulus operator to obtain the remainder  (n % 2): {(num % 2)}");

                char[] outputArr = num.ToString().ToCharArray();
                
                Console.WriteLine($"Use the ToCharArray() method to transform string to chararray: {new String(outputArr)}");

                Console.WriteLine("Use the ToCharArray() method to transform string to chararray");
                for (int i = 0; i < outputArr.Length; i++)
                {
                    Console.Write($"[{i}],[{outputArr[i]}];");
                }
            ...
            {
                Console.WriteLine("Can't parse sting to int");
            }
        }
        #endregion
        ...
        static void Morse_code()
        {
            ...
            char[] sosArr = sos.ToCharArray();
            foreach (char symbol in sosArr)
            {
                i = 0;
                foreach (string item in Dictionary_arr)
                {
                    if (item == symbol.ToString())
                    {
                        foreach (char charTobeep in Dictionary_arr[1,i])
                        {
                            if (charTobeep == '.')
                            {
